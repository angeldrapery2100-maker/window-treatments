...
const IMG_BASE = `${CDN_BASE}/hunter-douglas/pirouette`
...
function SwatchCollectionSection({ collection, onImg }: { collection: any; onImg: (s: string, caption?: string) => void }) {
  // 按对分组 (PDF原版每页左右两个)
  const pairs: any[][] = []
...
function ControlSystemsSection({ panels, sceneImage, sceneLabel, onImg }: {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Operating Systems - 3列横排，图片原始比例，文字在图片下方 */}
      <div>
...
/* ═══════════ MAIN ═══════════ */
export default function PirouetteDetailClient({ product, related, footer }: Props) {
  ...
  const layout = pirouetteLayout as any
