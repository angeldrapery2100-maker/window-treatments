...
const IMG_BASE = `${CDN_BASE}/hunter-douglas/applause`
      ...
      {scenes.map((scene, i) => (
        <div key={i} className={`flex flex-col ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6`}>
          <div className="flex-[3] cursor-zoom-in rounded-lg overflow-hidden relative" onClick={() => onImg(sectionImgs, i)}>
            <img src={`${IMG_BASE}/${scene.image}`} alt={scene.label || scene.text || 'Applause honeycomb shade scene'} className="w-full h-auto" loading="lazy" />
            {scene.label && (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {scenes.map((scene, i) => (
        <div key={i} className={`group rounded-lg overflow-hidden bg-gray-100 cursor-zoom-in relative ${i === 0 ? 'md:col-span-2' : ''}`}
          onClick={() => onImg(sectionImgs, i)}>
          <img src={`${IMG_BASE}/${scene.image}`} alt={scene.text || scene.label || 'Applause honeycomb shade photo'} className="w-full h-auto group-hover:opacity-95 transition-opacity" loading="lazy" />
...
/* ═══════════ MAIN ═══════════ */
export default function ApplauseDetailClient({ product, related, footer }: Props) {
  const [hoveredNav, setHoveredNav] = useState<string | null>(null)
  const [lbImages, setLbImages] = useState<LightboxImage[]>([])
  const [lbIndex, setLbIndex] = useState(-1)
  const layout = applauseLayout
